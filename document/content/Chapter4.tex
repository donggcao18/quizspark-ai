...
The package diagram of the system (Figure~\ref{fig:high_level_arch}) illustrates how the main front-end and back-end packages are organized and how they depend on each other to fulfill user requests. It shows the system as a set of logical layers and modules, highlighting separation of concerns, clear dependency directions, and overall maintainability.
...
\begin{itemize}
    \item \textbf{pages}: Route-level containers that model the main user flows (e.g., Dashboard, Questions Banks, Practice, Workspace). A page orchestrates data fetching and composes multiple UI components into a complete screen.
...
\begin{figure}[htbp]
    ...
    \caption{Component Diagram of QuizSpark AI System}
    \label{fig:component_diagram}
\end{figure}

The application is composed of several key components that work together to provide the desired functionality (see more in the Figure ~\ref{fig:component_diagram}).
...
\subsubsection{a. Screen Specifications and Layout}
...
\subsubsection{d. Navigation and Interaction Patterns}
...
The following diagram (Figure~\Ref{fig:erd_diagram}) illustrates the relationships between the key entities in the QuizSpark AI database schema.
\begin{figure}[http]
    \centering
