...
\subsubsection{a. AI Integration}
...
\subsubsection{b. Practice \& Quiz Module}
...
\subsubsection{c. Classroom Module}
The Classroom Module provides collaborative learning features, allowing teachers to organize students into virtual classrooms.

\paragraph{Core Components}
\begin{itemize}
    \item \textbf{ClassroomSection}: Defined in \texttt{classroom\_section.tsx}, this component displays a searchable list of classrooms the user belongs to. It fetches classroom data via the \texttt{getUserClassrooms} API and provides filtering by name or description.
    \item \textbf{ClassroomCreationDialog}: A modal component that allows teachers and admins to create new classrooms by specifying a name and description. Upon creation, the backend generates a unique join code.
    \item \textbf{JoinClassroomDialog}: Enables students to join existing classrooms by entering a 6-character alphanumeric join code.
    \item \textbf{ClassroomOverviewPage}: The detailed view for a specific classroom, displaying member lists with role badges, inline user search for invitations, and member management controls.
\end{itemize}

\paragraph{Classroom Interaction Flow}
The classroom workflow supports both code-based and invitation-based membership:
\begin{enumerate}
    \item \textbf{Classroom Creation}: Teachers create a classroom via the dialog; the system generates a unique join code.
    \item \textbf{Joining via Code}: Students enter the join code in \texttt{JoinClassroomDialog}, which calls the \texttt{POST /classrooms/join} endpoint.
    \item \textbf{Direct Invitation}: Teachers search for users by username and invite them directly via the \texttt{inviteToClassroom} API.
    \item \textbf{Member Display}: The overview page renders member cards with role indicators (Owner, Teacher, Student) and provides removal controls for authorized users.
\end{enumerate}

\subsection{Backend}
The backend, built with Spring Boot, implements the business logic and exposes RESTful endpoints.

\subsubsection{a. Practice \& Assessment Module}
...
\end{itemize}
\subsubsection{b. AI Intelligence Module}
...
\subsubsection{c. Classroom Module}
The Classroom Module implements collaborative features through the \texttt{ClassroomService} and \texttt{ClassroomController}, managing classroom lifecycles and membership.

\paragraph{Classroom Lifecycle Management}
The service handles complete classroom administration:
\begin{itemize}
    \item \textbf{Creation}: The \texttt{createClassroom} method creates a new \texttt{Classroom} entity with a unique 6-character alphanumeric join code generated by \texttt{generateUniqueJoinCode()}. The creator is automatically added as a \texttt{ClassroomMember} with the \texttt{OWNER} role.
    \item \textbf{Retrieval}: The \texttt{getClassrooms} method returns all classrooms where the authenticated user is a member, using the \texttt{ClassroomMemberRepository}.
    \item \textbf{Detail View}: The \texttt{getClassroomById} method validates membership before returning a \texttt{ClassroomDetailsDTO} containing full member information.
\end{itemize}

\paragraph{Membership Management}
The module supports flexible membership operations:
\begin{itemize}
    \item \textbf{Join via Code}: The \texttt{joinClassroom} method validates the join code, checks for existing membership, and adds the user as a \texttt{STUDENT}.
    \item \textbf{Direct Invitation}: The \texttt{inviteUser} method verifies that the inviter has \texttt{OWNER} or \texttt{TEACHER} privileges before adding the invitee as a \texttt{STUDENT}.
    \item \textbf{Role Hierarchy}: The system enforces a three-tier role system (\texttt{OWNER}, \texttt{TEACHER}, \texttt{STUDENT}) with corresponding permission checks.
\end{itemize}

\subsection{Database}
The application utilizes PostgreSQL for reliable data persistence, managed via Spring Data JPA.

\subsubsection{a. Data Persistence Implementation}
\begin{itemize}
...
\subsubsection{b. Entity Definitions}
...
This section provides a visual walkthrough of the QuizzSpark platform's core features, from initial access and authentication to advanced study management and classroom collaboration.

\subsection{Landing Page and Public Access}
The landing page serves as the entry point for users, offering an intuitive and clean interface that highlights the platform's core value.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp1.png}
    \caption{Home Page UI}
\end{figure}

When users visit the website, the home page is displayed by default. It is designed to provide essential information about the platform's capabilities while encouraging users to engage in testing and reinforcing their knowledge across various subjects.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp2.png}
    \caption{Essential Information}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp3.png}
    \caption{Knowledge Reinforcement Features}
\end{figure}

The platform supports public access, allowing guest users to practice quizzes without logging in. However, to access advanced features like progress tracking and personalized recommendations, a user account is required.

\subsection{User Authentication}
QuizzSpark provides a secure and flexible authentication system.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp4.png}
    \caption{Authentication Screen}
\end{figure}

Users can sign in or register using their Google account for a seamless experience or opt for the traditional method using an email and password.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp5.png}
    \caption{Account Options}
\end{figure}

During registration, users provide essential profile details including account type, username, full name, date of birth, and educational level to personalize their experience.

\subsection{Main Dashboard}
The personalized Dashboard provides a high-level overview of academic progress.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp6.png}
    \caption{Dashboard Overview}
\end{figure}

Key metrics displayed include the number of questions answered, active question banks, and the average time spent per quiz. This helps monitoring learning efficiency over time.

\subsection{Question Bank Management}
A core feature of QuizzSpark is the ability for users to curate and manage their own study materials.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp7.png}
    \caption{Management Interface}
\end{figure}

Users can create custom question banks by specifying a name and selecting visibility options (Public or Private).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp8.png}
    \caption{Bank Creation Success}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp9.png}
    \caption{Edit Options}
\end{figure}

The management console allows users to:
\begin{itemize}
    \item Modify bank details and toggle visibility.
    \item Set the bank status between ``Draft'' and ``Published''.
    \item Add or remove questions from the bank.
    \item Permanently delete a question bank if no longer needed.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp10.png}
    \caption{Deletion Option}
\end{figure}

\subsection{Quiz Practice and Results}
The practice module is designed for flexibility and immediate feedback.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp11.png}
    \caption{Practice Configuration}
\end{figure}

Before starting, users can customize their sessions with several options:
\begin{itemize}
    \item Total number of questions.
    \item Question and answer shuffling.
    \item Real-time answer reveals.
    \item Filtering by specific tags.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp12.png}
    \caption{Active Practice Session}
\end{figure}

During a session, users receive immediate visual feedback on their selections.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp13.png}
    \caption{Real-time Feedback}
\end{figure}

Upon completion, a detailed summary is displayed, showing the total correct answers, overall accuracy percentage, and total time taken.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp14.png}
    \caption{Results Summary}
\end{figure}

Users can also revisit and review their practice history to identify areas for improvement or retake a quiz to improve their score.

\subsection{Classroom Collaboration}
QuizzSpark facilitates collaborative learning through its Classroom feature.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp15.png}
    \caption{Classroom Feature}
\end{figure}

The classroom UI is built to foster a shared learning environment.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp16.png}
    \caption{Classroom UI}
\end{figure}

Once a classroom is created, the system automatically generates a unique join code.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/imp17.png}
    \caption{Classroom Creation and Access}
\end{figure}

Participants can join these classrooms using the shared code to access assigned materials and participate in group study sessions.
...
\subsubsection{a. Testing Objectives}
...
\subsubsection{c. Test Environment}
...
\subsubsection{d. Results Executive Summary}
...
\subsubsection{a. Shared Features (Student \& Teacher)}
\textbf{Register \& Login} \\
\begin{table}[H]
\centering
\begin{tabular}{|c|p{4.5cm}|p{5.5cm}|p{3.0cm}|}
\hline
...
\end{tabular}
\caption{Functional test cases for shared registration and login features in QuizSpark}
\label{tab:test-shared-register-login}
\end{table}

\clearpage
\textbf{Personal Quiz Management}
\begin{table}[H]
\centering
\begin{tabular}{|c|p{4.5cm}|p{5.5cm}|p{2.0cm}|}
\hline
	\textbf{ID} & \textbf{Test Scenario} & \textbf{Expected Result} & \textbf{Actual Result} \\ \hline
...
\end{tabular}
\caption{Functional test cases for shared personal quiz management features in QuizSpark}
\label{tab:test-shared-personal-quiz}
\end{table}
...
\end{tabular}
\caption{Functional test cases for shared leaderboard functionality in QuizSpark}
\label{tab:test-shared-leaderboard}
\end{table}

\textbf{Multi-Platform Access}
\begin{table}[H]
\centering
\begin{tabular}{|l|p{4.5cm}|p{4.5cm}|p{3.5cm}|}
\hline
...
\end{tabular}
\caption{Functional test cases for shared multi-platform access in QuizSpark}
\label{tab:test-shared-multiplatform}
\end{table}
...
\end{tabular}
\caption{Functional test cases for shared privacy \& settings features in QuizSpark}
\label{tab:test-shared-privacy-settings}
\end{table}

\clearpage
\textbf{AI Question Generation}
\begin{table}[H]
\centering
\begin{tabular}{|l|p{4.5cm}|p{4.5cm}|p{3.5cm}|}
\hline
...
\end{tabular}
\caption{Functional test cases for shared AI question generation features in QuizSpark}
\label{tab:test-shared-ai-question-generation}
\end{table}

\subsubsection{b.Teacher Features:}

\textbf{Manage Classes}
\begin{table}[H]
\centering
\begin{tabular}{|l|p{4.5cm}|p{4.5cm}|p{3.5cm}|}
\hline
...
\end{tabular}
\caption{Functional test cases for teacher class management features in QuizSpark}
\label{tab:test-teacher-manage-classes}
\end{table}

\textbf{Host Quiz Session}
\begin{table}[H]
\centering
\begin{tabular}{|l|p{4.5cm}|p{4.5cm}|p{3.5cm}|}
\hline
...
\end{tabular}
\caption{Functional test cases for teacher live quiz hosting in QuizSpark}
\label{tab:test-teacher-host-quiz}
\end{table}

\subsubsection{c. Student Features:}

\textbf{Join Quiz Session}
\begin{table}[H]
\centering
\begin{tabular}{|l|p{4.5cm}|p{4.5cm}|p{3.5cm}|}
\hline
...
\end{tabular}
\caption{Functional test cases for student live quiz session participation in QuizSpark}
\label{tab:test-student-join-session}
\end{table}

\clearpage
\textbf{Practice Quiz \& Results}
\begin{table}[H]
\centering
\begin{tabular}{|l|p{4.5cm}|p{4.5cm}|p{3.5cm}|}
\hline
...
\end{tabular}
\caption{Functional test cases for student practice quizzes and result reporting in QuizSpark}
\label{tab:test-student-practice-results}
\end{table}
