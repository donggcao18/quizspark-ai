...
\paragraph{Classroom Member}
\textbf{Key attributes:}
\begin{itemize}
    \item \textit{id}: Identifier of a classroom membership record.
    \item \textit{role}: Role of the user within the classroom.
    \item \textit{joined\_at}: Time when the user joined the classroom.
    \item \textit{id}: Unique identifier of a classroom.
    \item \textit{user\_id}: Unique identifier of a user.
\end{itemize}
\textbf{Purpose:} The Classroom Member entity represents the participation of users in classrooms, capturing membership information and user roles within each classroom.
...
% % The Entity-Relationship Diagram (ERD) illustrates the logical structure and relationships between all database entities. The diagram uses standard notation where rectangles represent entities, diamonds represent relationships, and lines indicate cardinality constraints.


% % Key relationship patterns visible in the ERD include:

% % \begin{itemize}
% %     \item \textbf{User-Centric Design}: The Users entity serves as a central hub, connecting to classrooms, question banks, and practice sessions, reflecting the user-focused nature of the application.
    
% %     \item \textbf{Hierarchical Content Organization}: Question banks contain questions, which are further categorized through tags, creating a flexible three-tier content hierarchy.
    
% %     \item \textbf{Practice Session Tracking}: The Practice entity acts as a container for user assessment activities, linking users, classrooms, and specific question-answer pairs through Practice\_Question.
    
% %     \item \textbf{Collaborative Learning Structure}: The Classroom and Classroom\_Member entities enable multi-user learning environments with role-based access control.
% % \end{itemize}

% % This database design provides a solid foundation for scalability, supporting efficient queries for common operations such as retrieving user practice history, fetching questions by tag, managing classroom memberships, and generating adaptive assessments based on user performance data.

\subsubsection{b. Database Schema}
